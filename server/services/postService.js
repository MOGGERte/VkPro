  ...
  async getUserPosts(userId) {
    return await db.task(async t => {
      ...
      const userPosts = await t.any(`
        SELECT p.id, p.text, p.created_at as "createdAt",
      ...
      const userLikes = await t.any(`
        SELECT post_id
        FROM likes
        WHERE post_id IN ($1:csv) AND user_id = $2
      `, [postIds, userId]);
      ...
      return userPosts.map(post => ({
        ...
        text: post.text,
        likeCount: (likeCounts.find(lc => lc.post_id === post.id) || { count: 0 }).count,
        commentCount: (commentCounts.find(cc => cc.post_id === post.id) || { count: 0 }).count,
        repostCount: (repostCounts.find(rc => rc.post_id === post.id) || { count: 0 }).count,
